...
\begin{remark}
\label{rem:alt}
Observe that equation (\ref{eq:Sp}) can be expressed as

\begin{equation}
\label{eq:altSp}
\frac{1+F_d(z)}{1-pz(1+F_d(z))} = (1+F_d(z))
  \sum_{n=0}^\infty \big( pz(1+F_d(z)) \big)^n.
\end{equation}

The term $pz(1+F_d(z))$ is the weighted generating function of
``substitution intervals'', defined as a substitution followed by nothing
or by an error-free interval. Expression (\ref{eq:altSp}) thus
highlights that a read can be decomposed as a (potentially empty)
error-free interval, followed by a sequence of substitution intervals.
Note that every substitution interval contains exactly one substitution.
This representation will be useful in section~\ref{sec:av} when deriving
the average number of substitutions.
\end{remark}
...
immediate result by recalling remark~\ref{rem:alt}, where we observed
...
However, expression (\ref{eq:ass3}) does not converge exponentially.
...
both the numerator and the denominator, which makes for the spectacular
cancellation summarized in the following proposition.

\begin{proposition}
The average number of substitutions in seedless reads is asymptotically
equivalent to
\begin{equation*}
\frac{k+1}{z_1F_d'(z_1)} =
\frac{(k+1)(1-qz_1)^2}{(d-1)(qz_1)^{d+1}-d(qz_1)^d+qz_1},
\end{equation*}

\noindent
where $z_1$ is the only positive root of the polynomial $1-pz(1+F_d(z))$.
\end{proposition}
