...
seeding methods}
%\author{
%\textsc{Guillaume Filion & Eduard Valera Zorita} \\ [1ex]
...
$q/2$, where $q = 1-p$, what is the weighted generating function of
sequences without \texttt{CG}?

First, we define the weighted generating functions of the individual
symbols. With natural notatoins we see that $C(z) = G(z) = pz/2$ and that
$A(z) = T(z) = qz/2$.
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.89]{sketch_seeding.pdf}
...
$I-M(z)$.  The weighted generating function of the reads is the same as
under the constant substitution model, and the total weights are all equal
to $1$.
...
\begin{equation*}
S(z) = \frac{1+(1-\delta)\big(qz+(1-\delta)(qz)^2 + \ldots +
...
Introducing insertions brings additional difficulties. First, they usually
come in bursts and are not even spread over the read. This is also the
case of deletions, but we could neglect it because this does not affect
the size of the interval (all deletions are error-only intervals of size
$0$). The distribution of insertions create a situation analogous to
example~\ref{ex:Sanger}, where the total weight of reads of size $k$ is no
longer equal to $1$. The second difficulty is that bursts come in two
distinct types: the closed bursts that are followed by a match or a
substitution, and the ``runaway'' bursts at the end of reads.

To model insertion bursts, we need to assign a probability $r$ to the
first insertion, and a probability $a > r$ to all subsequent insertions of
the burst. The burst stops with probability $1-a$ at each decoded
nucleotide, so closed insertion bursts have weighted generating function
...
We do not show $M_*(z) = (I-M(z))^{-1}$ because the expression is fairly
complex and we will not use it anyway.

$\lambda(z) = 1 - (p+a)z +(pa-(1-a)pr)z^2  +F(z) ((1-\delta)(ar+a-r)pz^2
+(a\delta-(1-\delta)p-(1-a)r)z-\delta)$

\begin{equation*}
1 + U(z) \cdot M_*(z) \cdot V = \frac{\left( 1-(a-r)z \right)
\left(1+(1-\delta)F(z) \right)}{\lambda(z)}.
\end{equation*}

\begin{equation}
R(z) = \frac{1-(a-r)z}{1-(1+a-r)z+(a-(1+pa)r+(1-a)r^2)z^2}.
\end{equation}

%sage: (((a - 1)*r^2 + (a*p + 1)*r - a)*z^2 + (a - r + 1)*z - 1).roots(z)
%[(-1/2*(a - r + sqrt((4*a - 3)*r^2 + a^2 + 2*(2*a*p - a + 1)*r - 2*a + 1)
%+ 1)/((a - 1)*r^2 + (a*p + 1)*r - a),
%  1),
% (-1/2*(a - r - sqrt((4*a - 3)*r^2 + a^2 + 2*(2*a*p - a + 1)*r - 2*a + 1)
%+ 1)/((a - 1)*r^2 + (a*p + 1)*r - a),
