...
\begin{equation}
\label{eq:altSp}
\sum_{n=0}^\infty \big( (1+F_d(z))pz \big)^n
  (1+F_d(z)).
\end{equation}
...
\begin{definition}
An \textbf{S-interval} is a possibly empty error-free interval, followed
by a substitution.
\end{definition}

\begin{figure}[h]
\centering
\includegraphics[scale=0.90]{sketch_S_intervals.pdf}
\caption{\textbf{Decomposition into S-intervals}. When
...
read can be decomposed as a sequence of S-intervals followed by
...
If the read starts with an S-interval of size $m$ ($1 \leq m \leq d$), it
consists of an error-free interval of size $m-1$, followed by a
substitution, followed by a seedless read of size $k-m$.  The probability
of such a read is $q^{m-1} \cdot p \cdot (C/z_1^{k-m+1} + \Delta)$. Since
the occurrence of seedless reads of size is $k$ is $C/z_1^{k+1}+\Delta$,
the probability that a seedless read starts with an S-interval of size $m$
is approximately equal to $pz_1(qz_1)^{m-1}$.
...
\begin{equation*}
  \sum_{n=0}^\infty \left(\frac{F_d(z)\big( \delta + (1-\delta)pz \big)}
  {1-pz} \right)^n \frac{1+(1-\delta)F_d(z)}{1-pz}.
\end{equation*}
...
respectively, which together make an SD-interval
(Figure~\ref{fig:sketchdelint}).

\begin{definition}
An \textbf{SD-interval} is a sequence of deletions (called the head),
...
in SD-intervals. \textit{Top:} SD-intervals consist of a
...
interval. The tail is either a deletion, or a substitution (not preceded
...
Say that a read starts with an SD-interval whose head and body have
