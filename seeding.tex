...
\newcommand{\smDEL}{\scriptstyle{\Delta}}
...
All the examples chosen so far indicated that the analytic combinatorics
estimates are accurate. This is not surprising because the estimates
derived here converge exponentially fast to the target value.
Propositions~\ref{th:ass} and \ref{th:avsub} show that the rate of
convergence is dominated by the ratio between the dominant singularity and
the second singularity. Convergence accelerates as the dominant
singularity stands out from the others, and \textit{vice versa}. The worst
case for the approximation is thus when the reads are small and when the
first two singularities have close \textit{moduli}.
