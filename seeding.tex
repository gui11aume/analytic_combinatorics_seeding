...
A transfer matrices is closely related to the adjacency matrix of some
directed graph. The nodes of the graph are the combinatorial objects and
the edges are the permitted transitions. Figure~\ref{fig:CG_transtions}
shows the graph assciated with the tansfer matrix above. The nodes are the
nucleotides. Only the \texttt{C} to \texttt{G} transition is forbidden.
The transfer matrix carries more information because it encodes the size
and weight of the objects through their weighted generating function.

\begin{figure}[h]
\centering
\includegraphics[scale=0.9]{CG_transitions.pdf}
\caption{\textbf{Graph of the transfer matrix}. The transfer matrix shown
in (\ref{eq:Mz_CG}) is related to a transition graph between
nucleotides, where all the transitions are allowed, except \texttt{C} to
\texttt{G}. A sequence without \texttt{CG} can be seen as a walk on this
graph.}
...
The vector $U(z) = (1, qz, (qz)^2, \ldots, (qz)^{d-2})$.
