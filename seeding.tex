...
$pz$ and that of correct nucleotides is $qz$, where $q=1-p$. The
weighted generating function of error-free intervals comes as

\begin{equation}
F(z) = qz + (qz)^2 + (qz)^2 + \ldots = \frac{qz}{1-qz}.
\end{equation}
...
Using the elementary rules of algebra, we find
...
Applying proposition~\ref{th:TM2WGF} with $U(z)$ the row vector equal to
$F(z), pz$ and $V$ the column vector equal to $(1,1)$, the weighted
generating function of reads comes as
...
reads of size $k$ is equal to $1$ for every finite $k$.
...
relatively unrealistic case where errors can be deletions or
susbtitutions. As in the case of uniform substitutions, we assume that
every call is false with a probability $p$ and true with a probability
$1-p=q$. But here, we also assume the ``space''  between consecutive
nucleotides can contain a deletion with probability $\delta$.
...
reason, we consider that deletions exist only between correct calls,
simplifying the problem without loss of generality.
Figure~\ref{fig:deletions} shows the transition probabilities between
errors and correct calls.
